...
- Update-Endpoint: `success` wurde fälschlich `true` gemeldet, wenn `shell_exec()` deaktiviert war oder fehlschlug (`null`-Rückgabe führte dazu, dass `str_contains()` `false` lieferte). Auf `Symfony\Component\Process\Process` umgestellt (nutzt `proc_open()`, das auf vielen Hosts noch aktiv ist, während `exec()`/`shell_exec()` gesperrt sind — u.a. weil Craft es selbst für den Sendmail-Mailversand braucht), inkl. `ExecutableFinder` zur automatischen Composer-Pfad-Suche (PATH + gängige Standardorte) und echtem Exit-Code statt Text-Suche für `success`
